...
# Создаём питомца с корректными полями
$petData = @{
    name = "Arty_for_delete"
    type = "Cat"  
...
# Удаление питомца
try {
    # $petId = $petresponse.pet_id
    $petId = $petresponse.pet_id
    Write-Host $petId
    $deleteResponse = Invoke-RestMethod -Uri "$API_URL/api/pets/$petId" `
        -Method Delete `
        -Headers @{
            "Authorization" = "Bearer $token"
        }

    Write-Host "Питомец удалён" -ForegroundColor Green
    $deleteResponse | ConvertTo-Json -Depth 4 | Write-Host
}
catch {
